  ...
  >
    ...
    >
      <span v-if="collapseTags && selected.length">
        <el-tag
          v-for="tag in shownTagsWhenCollapse"
          :key="getValueKey(tag)"
        ...
        <el-tag
          v-if="selected.length - shownTagsWhenCollapse.length"
          :closable="false"
          :size="collapseTagSize"
          type="info"
          disable-transitions
        >
          <span class="el-select__tags-text">+ {{ selected.length - shownTagsWhenCollapse.length }}</span>
        </el-tag>
      </span>
      <transition-group
...
// https://element.eleme.io/#/en-US/component/select#create-new-items
  ...
  export default {
    ...
    computed: {
      shownTagsWhenCollapse() {
        return this.selected.slice(0, this.collapseLimit);
      },
